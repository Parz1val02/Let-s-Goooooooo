...
var (
	...
	ErrInvalidDigitPlace = errors.New("ssn that starts with a 9 requires 7 or 9 in the fourth place")
)
...
func main() {
	...
	for i, v := range validateSSN {
		log.Printf("Validate data %#v %d of %d", v, i+1, len(validateSSN))
		ssn := strings.Replace(v, "-", "", -1)
...
func validPrefix(ssn string) error {
